...
Class DBOperations {
  ...
  public function sync_customer2($db){

    $sth = $db->prepare("[dbo].[sp1_mcp_allignment_new]");
    $result = $sth->execute();
    return $result;
  } 

  public function purge_database($db){

    $sth = $db->prepare("[dbo].[sp_purgingdb]");
    $result = $sth->execute();
    return $result;
  } 

  public function clear_inventory_unuploaded_by_salesman($db, $mdCode, $transDate){

    $sth = $db->prepare("EXEC [sp_exportstatus] @mdcode = '$mdCode', @transdate = '$transDate'");
    $result = $sth->execute();
    return $result;
  } 
  ...
  public function exec_sp_invoice($db, $date){

    $sth = $db->prepare("exec [sp_iDeliverInvoice] '$date'");
    $result = $sth->execute();
    return $result;
  } 

  public function sync_ideliver_deltaggging($db, $db_ideliver, $site){
    $convertedSite = '';

    if($site == 'EV00401'){
      $convertedSite = 'FDC_BOHOL';
    }else if($site == 'GM02201'){
      $convertedSite = 'FDC_QC';
    }else if($site == 'SM01901'){
      $convertedSite = 'FDC_GENSAN';
    }else if($site == 'WV00201'){
      $convertedSite = 'FDC_ROXAS';
    }else if($site == 'NM01401'){
      $convertedSite = 'FDC_CDO';
    }else if($site == 'EV00201'){
      $convertedSite = 'FDC_SAMAR';
    }else if($site == 'EV00301'){
      $convertedSite = 'FDC_LEYTE';
    }

    $res = $this->get_idlvr_sctrpdetails($convertedSite, $db, $db_ideliver);
    if($res){
      return 1;
    }
    return $res;
  ...
  public function get_idlvr_sctrpdetails($site, $db, $db_ideliver){

    $sql = "SELECT
              cID, PO_NUMBER, DATECREATED, cast(Last_modified as datetime) as DISPATCH_DATE, ORIGIN_TIMEIN, ORIGIN_TIMEOUT,
              DISTINATION_TIMEIN, cast(DISTINATION_TIMEOUT as datetime) as DELIVERY_DATE, RETURNING_TIMEIN, RETURNING_TIMEOUT,
              (SELECT TOP 1 cast(InvoiceDate as date) from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest.PO_NUMBER) as INVOICEDATE,
              (SELECT TOP 1 CurrencyValue from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest.PO_NUMBER) as INVOICEAMOUNT,
              (SELECT TOP 1 Salesperson from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest.PO_NUMBER) as SALESMAN,
              (SELECT TOP 1 Customer from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest.PO_NUMBER) as CUSTOMERCODE,
              MAINSTATUS, FO_STATUS, SITEID
          from 
              tblFreightRequest 
          where
              MAINSTATUS = 7 and FO_STATUS = 4 and PO_NUMBER != 'transportno_2321'
            and DRIVERID not in ( 
              'DR8961144518',
              'DR40273970310',
              'DR7429244920',
            ...
            and SITEID = '$site'
            and RETURNING_TIMEIN is not null
            and DISTINATION_TIMEOUT is not null
            and cast(DATECREATED as date) between '2023-01-01' and cast(getdate() as date)
            ...
            SELECT
              cID, PO_NUMBER, DATECREATED, cast(Last_modified as datetime) as DISPATCH_DATE, ORIGIN_TIMEIN, ORIGIN_TIMEOUT,
              DISTINATION_TIMEIN, cast(DISTINATION_TIMEOUT as datetime) as DELIVERY_DATE, RETURNING_TIMEIN, RETURNING_TIMEOUT,
              (SELECT TOP 1 cast(InvoiceDate as date) from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest_prev.PO_NUMBER) as INVOICEDATE,
              (SELECT TOP 1 CurrencyValue from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest_prev.PO_NUMBER) as INVOICEAMOUNT,
              (SELECT TOP 1 Salesperson from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest_prev.PO_NUMBER) as SALESMAN,
              (SELECT TOP 1 Customer from tmp_ArInvoice where tmp_ArInvoice.Invoice = tblFreightRequest_prev.PO_NUMBER) as CUSTOMERCODE,
              MAINSTATUS, FO_STATUS, SITEID
          from 
              tblFreightRequest_prev 
          where
              MAINSTATUS = 7 and FO_STATUS = 4 and PO_NUMBER != 'transportno_2321'
            and DRIVERID not in ( 
              'DR8961144518',
    ...
    $stmt= $db_ideliver->query($sql);
    $dataideliver= $stmt->fetchALL(PDO::FETCH_OBJ);

    $cmd = "INSERT INTO [dbo].[NPIDispatchDelivery]
                                         ([Salesperson]
                                        ,[Customer]
                                        ,[Invoice]
                                        ,[InvoiceDate]
        ...
        $sql_insert_mybuddy = $db->prepare($cmd);
      foreach ($dataideliver as $i => $row) {
        $sql_insert_mybuddy->execute([
                              $row->SALESMAN,
                              $row->CUSTOMERCODE,
                              $row->PO_NUMBER,
                              $row->INVOICEDATE,
  ...
  public function sync_ideliver_invoice($db, $db_ideliver, $site, $dateselected){
    $convertedSite = '';
    ...
    //clear previous data
    // $this->delete_previous_invoice_header($db_ideliver, $convertedSite);
    // $this->delete_previous_invoice_details($db_ideliver, $convertedSite);

    //sync invoice data
    $this->exec_sp_invoice($db, $dateselected);
    $header = $this->ideliver_invoice_header($db, $db_ideliver, $convertedSite, $dateselected);
    $details = $this->ideliver_invoice_details($db, $db_ideliver, $convertedSite, $dateselected);
    //  $deatils_new = $this->ideliver_invoice_Details_new($db, $db_ideliver, $dateselected, $site);

    //update export status
    //  $this->update_previous_invoice_header_all($db, $dateselected);
    //  $this->update_previous_invoice_details_all($db, $dateselected);

    return $details;
  }

  public function ideliver_invoice_header($db, $db_ideliver, $convertedSite, $invoiceDate){
    
    // $sql = "SELECT * from vIArInvoice";
    $sql = "SELECT top 10 * from tmpArInvoice where exportStatus is null and cast(InvoiceDate as date) = '$invoiceDate'";
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);

    // foreach ($db->query($sql) as $row) {

      $cmd = 'INSERT INTO [dbo].[tmp_ArInvoice]
        ([Aging]
        ,[Customer]
        ,[Name]
        ,[PhoneNumber]
    ...
    foreach ($data as $i => $row) {
        ...
        $sqlideliver = $db_ideliver->prepare($cmd);
        $sqlideliver->execute([$row->Aging
                              ,$row->Customer
                              ,$row->Name
                              ,$row->PhoneNumber
                              ,$row->Contact
                ...
                $this->update_previous_invoice_header($db, $row->InvoiceDate, $row->Invoice);
  ...
  public function ideliver_invoice_details($db, $db_ideliver, $site, $dateselected){
    
    // $sql = "SELECT * from vITrnDetails";
    $sql = "SELECT * from tmpInvDetails where exportStatus is null and InvoiceDate = '$dateselected'";
    
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);

    //original
    $cmd ='INSERT INTO [dbo].[tmp_TrnDetails]
    ([Invoice]
    ,[DetailLine]
    ,[StockCode]
    ,[ITEMDESC]
    ...
    VALUES(
     ...
     foreach ($data as $i => $row) {
        $sqlideliver = $db_ideliver->prepare($cmd);
        $sqlideliver->execute([$row->Invoice
                              ,$row->DetailLine
                              ,$row->StockCode
                              ,$row->ITEMDESC
                              ,$row->Area
      ...
      $this->update_previous_invoice_details($db, $row->Invoice, $row->StockCode);
        
    }

    // $stmt = $db_ideliver->prepare($cmd);
    // try{
    //   $db_ideliver->beginTransaction();
    //   foreach($data as $row)
    //   {
    //     $stmt->execute($row);
    //   }
  ...
  public function ideliver_invoice_Details_new($db, $db_ideliver, $dateselected, $site){
    //get value from mybuddy first
    $sql_get = "SELECT * from tmpInvDetails where exportStatus is null and InvoiceDate = '$dateselected'";
    
    $stmt_get = $db->query($sql_get);
    $data_get = $stmt_get->fetchALL(PDO::FETCH_OBJ);


    //loop value to insert
    $data = array();
    $f_Data = array();
    foreach ($data_get as $i => $row) {

      $data["DetailLine"] = $row->DetailLine;
      $data["StockCode"] = $row->StockCode;
      $data["ITEMDESC"] = $row->ITEMDESC;
      $data["Area"] = $row->Area;
      $data["QtyInvoiced"] = $row->QtyInvoiced;
      $data["Mass"] = $row->Mass;
      $data["Volume"] = $row->Volume;
      $data["NetSalesValue"] = $row->NetSalesValue;
      $data["DetTaxValue"] = $row->DetTaxValue;
      $data["CostValue"] = $row->CostValue;
    ...
    $db_ideliver->beginTransaction(); // also helps speed up your inserts.
    $new_insert_values = array();
    foreach($f_Data as $d){
        $question_marks[] = '('  . $this->placeholders('?', sizeof($d)) . ')';
        $new_insert_values = array_merge($new_insert_values, array_values($d));
    }
    
    $datafields = array('[Invoice]','[DetailLine]','[StockCode]','[ITEMDESC]',
                        '[Area]','[QtyInvoiced]','[Mass]','[Volume]','[NetSalesValue]','[TaxValue]','[CostValue]','[SITE]');
                        
    $sql = "INSERT INTO tmp_TrnDetails (" . implode(",", $datafields ) . ") VALUES " .
          implode(',', $question_marks);
    
    $stmt = $db_ideliver->prepare ($sql);
    $stmt->execute($new_insert_values);
    $db_ideliver->commit();
...
public function update_previous_invoice_header_all($db, $invDate){
  $cmd = 'UPDATE tmpArInvoice SET exportStatus = 1 where cast(InvoiceDate as date) = ?';
  $sql = $db->prepare($cmd);
  $result = $sql->execute([$invDate, $Invoice]);

  return $result;
}

  public function update_previous_invoice_header($db, $invDate, $Invoice){
    $cmd = 'UPDATE tmpArInvoice SET exportStatus = 1 where cast(InvoiceDate as date) = ? and Invoice = ?';
    $sql = $db->prepare($cmd);
    $result = $sql->execute([$invDate, $Invoice]);
  ...
  public function update_previous_invoice_details_all($db, $dateselected){
      $cmd = "UPDATE tmpInvDetails SET exportStatus = 1 where cast(InvoiceDate as date) = ?";
      $sql = $db->prepare($cmd);
      $result = $sql->execute([$dateselected]);
  
      return $result;
  }

  public function update_previous_invoice_details($db, $invoice, $stockCode){
    // $cmd = "UPDATE tmpInvDetails SET exportStatus = 1 where cast(InvoiceDate as date) = ?";
    $cmd = "UPDATE tmpInvDetails SET exportStatus = 1 where Invoice = ? and StockCode = ?";
    $sql = $db->prepare($cmd);
    $result = $sql->execute([$invoice, $stockCode]);

    return $result;
  }

  public function delete_previous_invoice_details($db, $site){
    $cmd = 'DELETE tmp_TrnDetails WHERE SITE = ?';
    $sql = $db->prepare($cmd);
    $result = $sql->execute([$site]);

    return $result;
  }

  public function disable_salesman($db, $mdCode){
    $sql = $db->prepare("UPDATE TBLUSER SET isActive = '0' where mdCode = ?");
    $result = $sql->execute([$mdCode]);
  ...
  public function sync_sfa_queues_stocktake($db, $date, $salesman){

    $delimeter = $this->salesmanCounter($db);
    $paramCounter = count($salesman);
    if((int)$paramCounter == (int)$delimeter){
      $sth = $db->prepare("[dbo].[sp_trnverifier2] @deliveryDate = '$date', @mdCode = 'ALL'");
      $result = $sth->execute();
    }else{
      foreach($salesman as $resultRow){
        $sth = $db->prepare("[dbo].[sp_trnverifier2] @deliveryDate = '$date', @mdCode = '$resultRow'");
        $result = $sth->execute();
      }
    }
  ...
  public function get_deliverytagging($db, $startDate, $endDate){
    $sql = "SELECT *, 
    (SELECT DATEDIFF(day, cast(InvoiceDate as date), cast(DeliveryDate as date)) AS DateDiff) as LEADTIME 
    FROM [NPIDispatchDelivery] where cast(DeliveryDate as date) between '$startDate' and '$endDate'";
    ...
    foreach ($db->query($sql) as $row) {
        ...
        $data["InvoiceValue"] = number_format($row["InvoiceValue"], 2); 
        $data["InvoiceDate"] = $row["InvoiceDate"];
        $data["DispatchDate"] = $row["DispatchDate"];
        $data["DeliveryDate"] = $row["DeliveryDate"]; 

        $data["InterfaceFlag"] = $row["InterfaceFlag"]; 
        $data["FileSequenceNo"] = $row["FileSequenceNo"]; 
        $data["DateInserted"] = $row["DateInserted"]; 
        $data["ExportStatus"] = $row["ExportStatus"];  

        $data["LEADTIME"] = $this->checkleadtime($row["LEADTIME"]);
  ...
  public function checkleadtime($stat){
    if($stat == 0){
      return '1';
    }
    return $stat;
  ...
  public function get_aio_token($db, $storeID){

    $sth = $db->query("SELECT TOKEN as token from TBLSTOREOWNER where STOREID = '$storeID'");
    $row = $sth->fetch();

    return $row['token'];
  }
	
  public function get_aio_transaction($db, $distCode){
      $sql = "SELECT DISTINCT a.*, b.*, c.refno, c.totalAmount, c.deliveryDate,
      (SELECT FULLNAME FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as STOREOWNER,
      (SELECT TOKEN FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as TOKEN,
      (SELECT PHONENUMBER FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as STORECONTNO,
      (SELECT count(*) as totalSku FROM TBLTRANSACTIONITEMS WHERE TBLTRANSACTIONITEMS.transactionID = c.transactionID) as TOTALSKU,
      c.transactionID
       from tblcustomer_ls a, tblstore b, tbltransaction c
      where a.storeID = b.storeID and b.storeID = c.storeID and a.FSOSYO_DIST_CODE = '$distCode' and c.SupplierID = 'NPI'
      ...
      // c.transactionID
      //  from tblcustomer_ls a, tblstore b, tbltransaction c
      // where a.storeID = b.storeID and b.storeID = c.storeID and c.FASTSOSYO_DIST_CODE = '$distCode' and c.SupplierID = 'NPI' and c.status = 0
      // order by c.deliveryDate desc";
      $stmt= $db->query($sql);
      $data= $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function get_aio_distributorsaccount($db, $distCode){
    $sql = "SELECT * from [TBLDISTRIBUTORSACCOUNT] where FSOSYO_DIST_CODE = '$distCode' and custCode not like '%TMP%'";
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);
...
//   public function get_sosyofeedback_res($db, $custCode){
//     $sql = "SELECT transdate, ACTIVATED, INCOMP_PN, NOSIGNAL, NOINTERNET, NOELECTRICITY, OWNEROUT, NOTTECHY, NOTINTRSTD, REMARKS from tblfeedback where custCode = '$custCode'";
//     $stmt= $db->query($sql);
//     $data= $stmt->fetchALL(PDO::FETCH_OBJ);
...
//   }
  ...
  public function get_sosyofeedback_res($db, $stat, $custCode){
    $sql = $db->query("SELECT ".$stat." as stat from tblfeedback where custCode = '$custCode'");
    $retval = $sql->fetch();
    $res = $retval['stat'];
  ...
  public function get_sosyofeedback($db_aio, $db, $distCode){
    $sql = "SELECT * from [TBLDISTRIBUTORSACCOUNT] 
    where FSOSYO_DIST_CODE = '$distCode' and custCode not like '%TMP%'
    -- where FSOSYO_DIST_CODE = '$distCode' and partnerID in ('630017', '630020', '630005', '630018', '630025', '630012', '630030', '630003', '630026')
    ";
    $f_Data = array();
    $data = array();
    foreach ($db_aio->query($sql) as $row) {
        $data["mdCode"] = $row["partnerID"];
        // $data["salesman"] = $this->get_salesmanname($db, $row["partnerID"]); 
        $data["custCode"] = $row["custCode"]; 
        $data["custName"] = $row["custName"];
        $data["transdate"] = $this->get_sosyofeedback_res($db_aio, 'TRANSDATE', $row["custCode"]);
        $data["activated"] = $this->get_sosyofeedback_res($db_aio, 'ACTIVATED', $row["custCode"]);
        $data["incompatible"] = $this->get_sosyofeedback_res($db_aio, 'INCOMP_PN', $row["custCode"]);
        $data["nosignal"] = $this->get_sosyofeedback_res($db_aio, 'NOSIGNAL', $row["custCode"]);
        $data["nointernet"] = $this->get_sosyofeedback_res($db_aio, 'NOINTERNET', $row["custCode"]);
        $data["noelectricity"] = $this->get_sosyofeedback_res($db_aio, 'NOELECTRICITY', $row["custCode"]);
        $data["ownerout"] = $this->get_sosyofeedback_res($db_aio, 'OWNEROUT', $row["custCode"]);
        $data["nottechy"] = $this->get_sosyofeedback_res($db_aio, 'NOTTECHY', $row["custCode"]);
        $data["notinterested"] = $this->get_sosyofeedback_res($db_aio, 'NOTINTRSTD', $row["custCode"]);
        $data["remarks"] = $this->get_sosyofeedback_res($db_aio, 'REMARKS', $row["custCode"]);
        $f_Data[] = $data;
    }
    return $f_Data;
  ...
  public function update_sosyofeedback($db_aio, $date, $salesman, $custCode, $customername, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $distcode){
      $checker = $this->check_feedback_already_inserted($db_aio, $custCode);
        // return $checker;

      if((int)$checker == 0){
         $ret = $this->insert_feedback($db_aio, $date, $salesman, $custCode, $customername, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $distcode);
         return $ret;
      }else if((int)$checker == 1){
        $ret2 = $this->update_feedback($db_aio, $date, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $custCode);
        return 'update';
      }
  ...
  public function insert_feedback($db_aio, $date, $salesman, $custCode, $customername, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $distcode){
    $sqltext = "INSERT into tblfeedback (TRANSDATE, SALESMAN, CUSTCODE, CUSTOMERNAME, ACTIVATED, INCOMP_PN, NOSIGNAL, NOINTERNET, NOELECTRICITY, OWNEROUT, NOTTECHY, NOTINTRSTD, REMARKS, DISTCODE,
    LASTUPDATED, STATUS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, getdate(), 1)";
    $sql = $db_aio->prepare($sqltext);
    $result = $sql->execute([$date, $salesman, $custCode, $customername, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $distcode]);
     return $result;
  }

  public function update_feedback($db_aio, $date, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $custCode){
    $sqltext = "UPDATE TBLFEEDBACK SET TRANSDATE = ?, ACTIVATED = ?, INCOMP_PN = ?, NOSIGNAL = ?, NOINTERNET = ?, NOELECTRICITY = ?, OWNEROUT = ?, NOTTECHY = ?, NOTINTRSTD = ?, REMARKS = ?, LASTUPDATED = getDate()
    WHERE custCode = ?";
    $sql = $db_aio->prepare($sqltext);
    $result = $sql->execute([$date, $activated, $incomp, $nosignal, $nointernet, $noelectric, $ownerout, $nottechy, $notinserted, $remarks, $custCode]);
     return $result;
  }


  public function check_feedback_already_inserted($db, $custCode){
    $sql = $db->query("SELECT count(*) as checker from tblfeedback where custCode = '$custCode'");
    $retval = $sql->fetch();
    $res = $retval['checker'];
  ...
  public function get_salesmanname($db, $mdCode){
    $sql = $db->query("SELECT mdName as salesmanname from tblUser where mdCode = '$mdCode'");
    $retval = $sql->fetch();
    $res = $retval['salesmanname'];

    return $res;
  }

  public function check_preselectedAccts($db, $custCode, $mdCode, $distCode){
    $sql = "SELECT * from [TBLDISTRIBUTORSACCOUNT] where custCode = '$custCode' and partnerID = '$mdCode' and FSOSYO_DIST_CODE = '$distCode' and isLInkedTo = '1'";
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function update_preselectedAccrts($db, $custCode, $mdCode, $distCode){
      $sql = "UPDATE TBLDISTRIBUTORSACCOUNT SET isLInkedTo = '1', LinkedDate = getDate() where custCode = '$custCode' and partnerID = '$mdCode' and FSOSYO_DIST_CODE = '$distCode'";
      $sth = $db->prepare($sql);
      $result = $sth->execute();
      return $result;
  }

  public function get_aio_transaction_temp($db){
    $sql = "SELECT DISTINCT c.refno, c.totalAmount, c.deliveryDate as transactionDelDate, a.*, b.*, 
    (SELECT FULLNAME FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as STOREOWNER,
    (SELECT TOKEN FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as TOKEN,
    (SELECT PHONENUMBER FROM TBLSTOREOWNER WHERE TBLSTOREOWNER.STOREID = a.STOREID) as STORECONTNO,
    (SELECT count(*) as totalSku FROM TBLTRANSACTIONITEMS WHERE TBLTRANSACTIONITEMS.transactionID = c.transactionID) as TOTALSKU
     from tblcustomer_ls a, tblstore b, tbltransaction c
    where a.storeID = b.storeID and b.storeID = c.storeID and b.isDummy = 0";
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function update_aiotransaction_mdcode($dbaio, $mdCode, $transactionID, $custCode){

    $sql ="UPDATE tblTransaction set mdCode = ?, custCode = ? where transactionID = ? and SUPPLIERID = 'NPI'";
  
    $sth = $dbaio->prepare($sql);
    $result = $sth->execute([$mdCode, $custCode, $transactionID]);
    return $result;
  }
	
 public function update_aiocustomerls_mdcode($dbaio, $mdCode, $storeID, $custCode){

    $sql ="UPDATE tblcustomer_ls set partnerID = ?, custCode = ?, status = ? where STOREID = ? and SupplierCode = 'NPI'";

    $sth = $dbaio->prepare($sql);
    $result = $sth->execute([$mdCode, $custCode, '2', $storeID]);
    return $result;
 }
	
 public function getAIOTransactionID($db, $storeID, $supplierID){
	    $sth = $db->query("SELECT transactionID from tblTransaction where storeID = '$storeID' and SUPPLIERID = '$supplierID' and mdCode = ''");
	 //$sth = $db->query("SELECT transactionID as trans from tblTransaction where storeID = '19133991tNz2vMH420210918'");
    $row = $sth->fetch();

    return (int)$row['transactionID'];
  ...
  public function get_companyname($db){
    ...
    $sql = "SELECT company, DIST_CD, DBNAME from tblCompany";
    $output = array();
    foreach ($db->query($sql) as $row) {
      $data['company'] = $row['company'];
      $data['DIST_CD'] = $row['DIST_CD'];
      $data['DIST_INDI'] = $row['DBNAME'];
      $output[] = $data;
    }//foreach
  ...
  public function sent_details_tosalesman($db, $contactno, $transactionID){

    $sql ="exec [dbo].[sp_smsorderdetails] @transactionID = ?, @mobileNo = ?";  

    $sth = $db->prepare($sql);
    $result = $sth->execute([$transactionID, $contactno]);
    return $result;
  }
      ...
      $datastring = '';
        foreach($salesman as $resultRow){
          $sql = "SELECT mdCode,mdName, (select Type from tblSalesperson s1 where s1.mdCode = x1.mdCode) as Type,
                  ...
                  from
                    tmpsalessummarydigimap with(nolock)
                  where
                    mdCode <> '9999'
  ...
  public function get_all_aio_salesman($db, $db_cloud, $mdCode){

    if($mdCode == null || $mdCode == 'null'){
      return '---';
    }else{
      $sth = "SELECT mdCode, mdName as salesman, contactCellNumber from tblUser where mdCode = '$mdCode'";
      $result = array();
      $data = array();
        foreach ($db->query($sth) as $row) {
           $result["Salesman"] = $row["salesman"];
           $result["mdCode"] = $row["mdCode"];
           $result["phonenumber"] = $row["contactCellNumber"];
           $result["longitude"] = $this->get_salesman_last_location_lng($db_cloud, $mdCode);
           $result["latitude"] = $this->get_salesman_last_location_lat($db_cloud, $mdCode);
           $data[] = $result;
        }
  ...
  public function get_salesman_last_location_lat($db, $mdCode){

    $sql = "SELECT latitude from tblSFAUsers where mdCode = '$mdCode'";
    $result= $db->query($sql);
    $ret = $result->fetch();
    $res = $ret['latitude'];
  ...
  public function get_salesman_last_location_lng($db, $mdCode){

    $sql = "SELECT longitude from tblSFAUsers where mdCode = '$mdCode'";
    $result= $db->query($sql);
    $ret = $result->fetch();
    $res = $ret['longitude'];
  ...
  public function get_dashboard_markers($db){
     $date = date('Y-m-d');
         $sql = "SELECT [Salesman]
                     ...
                     ,[CustomerID]
                     ,[address]
                   FROM [dbo].[vSELLOUT] with(nolock) WHERE CAST(deliveryDate as date) = cast(getdate() as date)
                   ORDER BY Salesman";
        // $sql = "SELECT [Salesman]
        //              ,[Customer]
        //              ,a.[longitude]
  ...
  public function get_conn_sites($db){
    $sql = "SELECT * from [.ini]";
    $stmt= $db->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function get_totalsales_dashboard_local($db){
    date_default_timezone_set('Asia/Manila');
        ...
        $sth = "SELECT 
                  sum(totalAmount) as total, count(distinct mdCode) as salesman, count(*) as indicator 
               from 
                  tblTransaction with(nolock) 
                where 
                  CAST(deliveryDate as date) = '$newdate' and isVerified = 1 and transstat = 0 and mdCode <> '9999'";
        $result = array();
        $data = array();
        foreach ($db->query($sth) as $row) {
             $result["indicator"] = $row["indicator"];
  ...
  public function get_salesman_images($db){
    $sth = "SELECT mdCode, thumbnail from tblUser";
    $result = array();
    $data = array();
        foreach ($db->query($sth) as $row) {
            $result["mdCode"] = $row["mdCode"];
  ...
  public function get_dsr_data($db, $salesman, $date){
  	...
  	//$sql = "select * from vSELLOUT_WTHNOCALLS where mdCode = '$salesman' and cast(deliveryDate as date) = '$date'";
	  $sql = "select * from (
	SELECT t1.deliveryDate,Sales, 
	(case when len(Customer) < 10 then (select concat(custCode,' ',custName) from tblCustomer s2 where s2.custCode = t1.CustomerID) else Customer end) Customer,
	isnull((select refno from tblTransaction s1 where s1.transactionID = t1.transactionID and s1.mdCode = t1.mdCode),'-') refno,#SKU, t1.Notation, 
	CustomerID custCodeParams,t1.mdCode , 
	isnull((select upTime from tblTransaction s1 where s1.transactionID = t1.transactionID and s1.mdCode = t1.mdCode),'-') upTime, 
		t1.transactionID as transID
	from vSELLOUT_WTHNOCALLS t1
	where t1.mdCode = '$salesman' and CAST(t1.deliveryDate as date) = '$date'
) x1 
Order By x1.deliveryDate";
  ...
  public function get_dsr_average_data($db, $salesman, $date){
    $sql = $db->query("SELECT sum(cast(totalAmount as float)) as average from tblTransaction where transstat != 1 and mdCode = '$salesman' and cast(deliveryDate as date) = '$date'");
        $result = $sql->fetch();
        return $result;
  }
  ...
  public function get_sellingdays_data($db, $salesman, $date, $numericMonth){
    $actualDate = $date->format('d');
        ...
        //  $trnDay = $db->query("SELECT trnDay from tblTarget WHERE trnMonth = '$numericMonth' and mdCode = '$salesman'");
        // $resulttrnDay = $trnDay->fetch();

        $sql1 = "SELECT trnDay as trnDay from tblTarget WHERE trnMonth = '$numericMonth' and mdCode = '$salesman'";
        $result1= $db->query($sql1);
        $trnDay = $result1->fetchColumn();

        $sql2 = "SELECT tAmount as tAmount from tblTarget WHERE trnMonth = '$numericMonth' and mdCode = '$salesman'";
        $result2= $db->query($sql2);
        $tAmount = $result2->fetchColumn();

        // $tAmount = $db->query("SELECT tAmount from tblTarget WHERE trnMonth = '$numericMonth' and mdCode = '$salesman'");
        // $resulttAmount= $tAmount->fetch();
        ...
        return array("day" => $trnDay, "amount" => $tAmount,
                      "actualDays" => $actualDays, "mtdSales" => $resultMTDsales);
  }
  ...
  public function get_stockrequest_filter_data($db, $mdCode, $refNo){
    //original query
        ...
        $sql = "SELECT * from vStockRequest where mdCode = '$mdCode' and refNo = '$refNo' and status = 'VERIFIED'";
        $res_tbody = array();
        foreach ($db->query($sql) as $row) {
          $data["stockCode"] = $row["stockCode"];
          $data["Description"] = $row["Description"]; 
  ...
  public function exec_sync_logs($db, $description, $isLock, $lockby){
   
      // $statement = "INSERT INTO [tblSync] (Description, isLock, LockBy, syncStarted)
      // values (?, ?, ?, getdate())";

      // $sql = $db->prepare($statement);
      // $result = $sql->execute([$description, 1, $lockby]);
  ...
  public function get_salesman_maintenance_list($db, $db_cloud){
    $sql = "SELECT CONCAT(mdSalesmancode, '_', mdName) AS sname, ishybrid,
      mdCode, contactCellNumber, mdUserCreated, eodNumber1, eodNumber2, mdPassword, mdLevel,
      customerLastDateReset, priceCode, mdColor, geoLocking, isGEOReset, siteCode, Password1, PreRouteCL,
	  PostRouteCL, StockTakeCL, EOD, DefaultOrdType, stkRequired, stkList, calltime,
      loadingCap, isNCRestricted, ImmediateHead, DisableOTP, SalesmanType from tblUser order by mdCode";
	  
        $salesmanlist = array();
       foreach ($db->query($sql) as $row) {
		   $aioactivated = $this->isfastsosyopartner($db_cloud, $row['mdCode']);
              $data["sname"] = $row["sname"];
              $data["ishybrid"] = $row["ishybrid"]; 
              $data["mdCode"] = $row["mdCode"]; 
              $data["contactCellNumber"] = $row['contactCellNumber'];
              $data["mdUserCreated"] = $row['mdUserCreated'];
              $data["eodNumber1"] = $row['eodNumber1'];
              $data["eodNumber2"] =  $row['eodNumber2'];
              $data["mdPassword"] = $row['mdPassword'];
		   
 			        $data["mdLevel"] = $row['mdLevel'];
              $data["customerLastDateReset"] = $row['customerLastDateReset'];
              $data["priceCode"] = $row['priceCode'];
		   
		          $data["mdColor"] = $row['mdColor'];
              $data["geoLocking"] = $row['geoLocking'];
              $data["isGEOReset"] = $row['isGEOReset'];
		   
 			        $data["siteCode"] = $row['siteCode'];
              $data["Password1"] = $row['Password1'];
              $data["PreRouteCL"] = $row['PreRouteCL'];
		   	      $data["PostRouteCL"] = $row['PostRouteCL'];
              $data["StockTakeCL"] = $row['StockTakeCL'];
              $data["EOD"] = $row['EOD'];
		          $data["fastsosyopartner"] = $aioactivated;
		          $data["DefaultOrdType"] = $row['DefaultOrdType'];
              $data["stkRequired"] = $row['stkRequired'];
              $data["stkList"] = $row['stkList'];

              $data["calltime"] =  $row['calltime'];
              $data["loadingCap"] = $row['loadingCap'];
              $data["newCustomer"] = $row['isNCRestricted'];
              $data["ImmediateHead"] = $row['ImmediateHead'];
              $data["DisableOTP"] = $row['DisableOTP'];
              $data["SalesmanType"] = $row['SalesmanType'];
  ...
  public function get_salesman_base64Images($db){
    $sql = "SELECT thumbnail as salesmanImage, mdCode from tbluser where token is not null";
     $stmt= $db->query($sql);
     $imageList = $stmt->fetchALL(PDO::FETCH_OBJ);
     return $imageList;
 }

 public function check_salesman_number($db, $contact, $mdCode){

  $sql = $db->query("SELECT mdCode as checker from tbluser where contactCellNumber = '$contact'");
  $retval = $sql->fetch();
  $res = $retval['checker'];
  ...
  public function exec_update_salesman_maintenance($db, $geolocking, $mdpassword, $password1, $mdlevel, 
  $eodNumber1, $eodNumber2, $contactcell, $color, $priceCode, $thumbnail, $defordsel, $stklist, $stkreq, 
  $pre, $post, $stack, $oedcl, $mdCode, $calltime, $capacity, $ishybrid, $newCustomer, $supervisor, $otpdisabled, $SalesmanTypeData){
    $sql = "UPDATE tblUser 
              SET stocktakecl = '$stack',
                   ...
                   loadingCap = '$capacity',
				           calltime = '$calltime',
                   isNCRestricted = '$newCustomer',
                   ImmediateHead = '$supervisor',
                   DisableOTP = '$otpdisabled',
                   SalesmanType = '$SalesmanTypeData'
              WHERE mdCode = '$mdCode'";
      $res = $db->prepare($sql); 
      $res->execute();  
  ...
  public function exec_stockcard_update_open($db, $mdCode, $stockCode, $refNo, $transDate){
	  $newRef = str_replace("'", "", $refNo);
      $sql = $db->prepare("UPDATE tblStockcard set isVerified = '1' WHERE refNo = ?
        and mdCode = ?");//" and cast(transDate as date) = cast('$transDate' as date)");
      $result = $sql->execute([$newRef, $mdCode]);
  ...
  public function exec_stockcard_update_verified($db, $mdCode, $stockCode, $refNo, $transDate){
	  $newRef = str_replace("'", "", $refNo);
      $sql = $db->prepare("UPDATE tblStockcard set rStat = '1' WHERE refNo = ?
        and mdCode = ?"); //and cast(transDate sa date) = cast('$date' as date)");
      $result = $sql->execute([$newRef, $mdCode]);
  ...
  public function get_unproductive_report($db, $start, $end){
    $key = 'SuperSecretKey';

    $sql = "SELECT * FROM tblCalls where cast(trnDate as date) between '$start' and '$end'";
    $f_Data = array();
    foreach ($db->query($sql) as $row) {
        
          $data["mdcode"] = $row["mdCode"].'_'.$this->get_salesmanname($db, $row['mdCode']);
          $data["trnDate"] = $row["trnDate"];
          $data["sentDate"] = $row["sentDate"]; 
          $data["rCode"] = $row["rCode"];
          $data["remarks"] = $row['remarks'];
          $data["custCode"] = $row['custCode'];
          $data["reasonCode"] = $this->getReaonCode($db, $row['rCode']);
          $data["customerName"] = $this->getCustomerName_custCode($db, $row['custCode']);
          $data["transactionID"] = $row['transactionID'];
         $f_Data[] = $data;
      }
    ...
    //  $encrypted = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $key, 'jhusnkikunski', MCRYPT_MODE_CFB);
    //  $decrypted = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $key, $encrypted, MCRYPT_MODE_CFB);
      
    // return urlencode($decrypted);
  }
  public function get_salesreport_data($db, $start, $end){
    /*$sql = "SELECT [Salesman]
        ...
        foreach ($db->query($sql) as $row) {
              ...
              $data["vouchercode"] = $row['PromoCode'];
              $data["voucherval"] = $row['PromoValue'];
             $f_Data[] = $data;
          }
  ...
  public function get_sync_report_data($db, $start, $end){
      $sql = "SELECT * FROM vSyncLogs WHERE cast(datesync as date) between '$start' and '$end'";
      $stmt= $db->query($sql);
  ...
  public function get_bank_list($db){
      $sql = "SELECT * from [dbo].[tblBank] order by bankname";
      $stmt= $db->query($sql);
      $chardata = $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function get_customertagging_data($db, $start){
  ...
  public function stock_stake_reports($db, $startDate, $endDate, $salesman){
      ...
      $sql = "SELECT STKDATE,SALESMAN,CUSTOMER,CUSTOMERCHANNEL,STAKENO,
      (select count(transactionID) FROM [O_TXN_STKPRD] as s1 with (nolock) where s1.transactionID = x1.transactionID ) as STKREQUIRED
      ,(select count(PRD_QTY1) from [O_TXN_STKPRD] as s1 with (nolock) where s1.transactionID = x1.transactionID and PRD_QTY1 > 0) as STKAVAILABLE
        ,(select count(PRD_QTY1) from [O_TXN_STKPRD] as s1 with (nolock) where s1.transactionID = x1.transactionID and PRD_QTY1 < 1) as STKOUTOFSTOCK
    from 
    (SELECT distinct cast(d.[TXN_DT] as date) as STKDATE,
              a.SLSMAN_CD+'_'+b.mdName as SALESMAN,
              c.custCode+'_'+c.custName as CUSTOMER,
              c.custType as CUSTOMERCHANNEL,
              a.TXN_NO as STAKENO, a.transactionID
          from [dbo].[O_TXN_STKPRD] a with (nolock), tblUser b with (nolock), tblCustomer c with (nolock), [O_TXN_STKHDR] d with (nolock)
          where a.SLSMAN_cD = b.mdSalesmancode and
          a.transactionID = d.transactionID and
          d.[CUST_CD] = c.custCode and 
          cast(d.[TXN_DT] as date) BETWEEN '$startDate' and '$endDate' and
  ...
  // ) x1";
    ...
    $sql = "EXEC sp_stocktakereports '$startDate', '$endDate'";
    $res = array();
     foreach ($db->query($sql) as $row) {
           $data["STKDATE"] = $row["STKDATE"];
           $data["SALESMAN"] = $row["SALESMAN"];
  ...
  public function tagging_customer_list($db){
    ...
    ,t3.mdSalesmancode + '_' + t3.mdName salesmanAssign, t2.DefaultOrdType
    from tblCustomer t1, tblCustomerImage t2, tblUser t3
    where t1.custCode = t2.custCode and t1.mdCode = t3.mdCode and t3.DefaultOrdType = 'H'";
  ...
  public function getCustomerName($db, $custCode){
      $sql = "SELECT custCode, custName from tblCustomer where custcode = '$custCode'";
      $stmt= $db->query($sql);
      $data = $stmt->fetchALL(PDO::FETCH_OBJ);
      return $data;
  }

  public function getCustomerName_custCode($db, $custCode){
    $sth = $db->query("SELECT custCode+' '+custName as CUSTOMERNAME from tblCustomer where custcode = '$custCode'");
    $res = $sth->fetch();
    return $res['CUSTOMERNAME'];
  }

  public function getReaonCode($db, $rsCode){
    $sth = $db->query("SELECT rCode+' - '+Description as reasonCode from tblReason where rCode = '$rsCode'");
    $res = $sth->fetch();
  ...
  public function get_customerList_aio($db, $mdCode){
    $sql = "SELECT custCode, custName from tblCustomer where mdCode = '$mdCode'";
    $stmt= $db->query($sql);
    $data = $stmt->fetchALL(PDO::FETCH_OBJ);
    return $data;
}
  ...
  public function isfastsosyopartner($db_cloud, $mdCode){
  $sth = $db_cloud->query("SELECT count(*) as existing from tblSFAUsers where mdCode = '$mdCode' and FASTSosyoEnabled = '1'");
    $res = $sth->fetch();
    return $res['existing'];
  }

  public function getCustomerName_fetch($db, $custCode){
    $sth = $db->query("SELECT custName as storename from tblCustomer where custcode = '$custCode'");
    $res = $sth->fetch();
 ...
 public function exec_insert_sfauser_mbuddy_linking($db_cloud, $mdCode, $mdName, $isLeadPartners, $contactCellNumber, $priceCode){

    $sql = $db_cloud->prepare("UPDATE [tblSFAUsers] set
							FASTSosyoEnabled = ?,
							mdName = ?,
							contactCellNumber = ?,
							priceCode = ?
							where mdCode = ?");
    $result = $sql->execute([$isLeadPartners, $mdName, $contactCellNumber, $priceCode, $mdCode]);
    $datastring = '';
    if($result){
      $datastring = 1;
    }else{
  ...
  public function exec_salesman_insert($db, $db_cloud, $mdName, $mdPassword, $contCellNum, $mdColor, $eod1, $eod2,$calltime, $priceCode, $defOrdType, $mdsalesmancode, $capacity, $supervisor, $salesmanType){
    ...
    if($checker == 1){
      ...
      $sql = $db->prepare("INSERT into [tblUser] 
                ...
                Values ('$mdCode', 
                        ...
                        getDate(), 
                        '$mdsalesmancode', 
                        '$mdName', 'Y', '$eod1', '$eod2', '$contCellNum',
                         '$mdColor', '123.850508', '9.671053', '1234', '1', 
                         '$convTime', '$priceCode', '$defOrdType', '15', '$capacity', '$supervisor', '$salesmanType')");
        $result = $sql->execute();
        if($result){
          $datastring = 1;
  ...
  public function sync_salesman_Details_sosyo($db_aio, $db){

    $getsitename = $this->getvsalesman_sitename($db);
    //remove existing sosyo records
    $this->delete_persite_salesman_Sosyo($db_aio, $getsitename);

    //start populating
    $sql = "SELECT * from vSalemanDetails";
    $res = array();
    foreach ($db->query($sql) as $row) {
      $this->insert_salesman_Sosyo($db_aio, $row["Principal"], $row["SiteName"], $row["mdCode"], $row['mdSalesmancode'], $row['mdName'], $row['MCP'], $row['DefaultOrdType'], $row['priceCode'], $row['eodNumber2'], $row['contactCellNumber'], $row['ImmediateHead']);
    }
    return 1;
  }


  public function sync_sweeper_Transaction($db_aio, $db){

    //get site sweeper
    $sql = "SELECT mdCode from tbluser where DefaultOrdType = 'S'";
    $res = array();
    foreach ($db->query($sql) as $row) {
     $this->get_sosyo_sweeper_transaction($db, $db_aio, $row["mdCode"]);
  ...
  public function get_sosyo_sweeper_transaction($db, $db_aio, $mdCode){
    //get site sweeper transaction
    $sql = "SELECT custCode, soNumber from tblTransaction where mdCode = '$mdCode'";
    $res = array();
    foreach ($db->query($sql) as $row) {
      $this->update_sosyo_sweeper_transaction($db_aio, $mdCode, $row["soNumber"]);
    }
  }

  public function update_sosyo_sweeper_transaction($db_aio, $sweeperMdCode, $transactionID){
    //update sosyo transaction
    $sql ="UPDATE tblTransaction set ASND_SWEEPER = ? where transactionID = ?";  
  
    $sth = $db_aio->prepare($sql);
    $result = $sth->execute([$sweeperMdCode, $transactionID]);
  
    return $result;
  }

  public function getvsalesman_sitename($db){
    $sth = $db->query("SELECT top 1 SiteName from vSalemanDetails");
    $res = $sth->fetch();
    return $res['SiteName'];
  }

  public function insert_salesman_Sosyo($db_aio, $principal, $sitename, $mdcode, $salesmancode, $mdname, $mcp, $defaultord, $priceCode, $eodnumber2, $contno, $immediatehead){
      $sql ="INSERT into tblsalesperson (Principal, Sitename, mdCode, mdSalesmancode, mdName, MCP, DefaultOrdType, priceCode, eodNumber2, contactCellNumber, ImmediateHead, lastUpdated) 
      values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, getDate())";  
    
      $sth = $db_aio->prepare($sql);
      $result = $sth->execute([$principal, $sitename, $mdcode, $salesmancode, $mdname, $mcp, $defaultord, $priceCode, $eodnumber2, $contno, $immediatehead]);
    
      return $result;
  }

  public function delete_persite_salesman_Sosyo($db_aio, $sitename){
    $sql ="DELETE tblsalesperson where SiteName = ?";  
  
    $sth = $db_aio->prepare($sql);
    $result = $sth->execute([$sitename]);
  ...
  public function dcr_data_report($db, $start, $end, $mdCode){
    $sql = "SELECT a.*, b.mdName, b.mdSalesmancode from tblCollection a, tblUser b
           ...
           and a.transstat = 0 order by ORNumber asc";// and a.custCode = c.custCode";
    $res = array();
    foreach ($db->query($sql) as $row) {
  ...
  public function collection_print_data($db, $start, $end, $mdCode){
    ...
    $res = array();
    foreach ($db->query($sql) as $row) {
      $custName = $this->getCustomerName_fetch($db, $row["custCode"]);
      $data["transactionID"] = $row["transactionID"];
      $data["trnDate"] = $row["trnDate"];
      $data["sendDate"] = $row["sendDate"];
      $data["mdCode"] = $row["mdCode"];
      $data["custCode"] = $row["custCode"];
      $data["custName"] = $row["custCode"].' '.$custName;
      $data["pType"] = $row["pType"];
      $data["ORNumber"] = $row["ORNumber"];
      $data["InvoiceNumber"] = $row["InvoiceNumber"];
      $data["Amount"] = round($row["Amount"], 2);
      $data["Amountraw"] = $row["Amount"];
      $data["BankCode"] =$row["BankCode"];
      $data["CheckNo"] = $row["CheckNo"];
      $data["CheckDate"] = $row["CheckDate"];
  ...
  public function exec_broadcast_sirroy($db, $phoneNumber, $message, $smstype){
    $sql ="INSERT into [eFastVITAL].dbo.[tblSMSOutbox] ([oDateTimeIn], [oMobileNo], [oSMSTYPE], [oRepliedMSG], [oClientID], [oPriority]) 
    values (getdate(), ?, ?, ?, ?, ?)";  
  
    $sth = $db->prepare($sql);
    $result = $sth->execute([$phoneNumber, $smstype, $message, 'Fastsosyo', '1']);
  
    return $result;
  
  }

  public function exec_insert_bank($db, $bankcode, $bankname){
    $sql ="INSERT into [tblBank] (bankCode, bankName,lastUpdated)  values (?, ?, GETDATE())";  
  
    $sth = $db->prepare($sql);
    $result = $sth->execute([$bankcode, $bankname]);
  
    return $result;
  
  }

  public function get_cmf_data($db, $start, $end){

    $sql = "SELECT * from tblCMF where cast(REQ_DATE as date) between '$start' and '$end'";
    $res = array();
    foreach ($db->query($sql) as $row) {

      $address = $row["SHP_MUNICIPALITY_CITY_PROV"].', '.$row["SHP_STREET_BRGY"];
      $data["CMFID"] = $row["CMFID"];
      $data["CUSTCODE"] = $this->emptyColumnChecker($row["CUSTCODE"], 1);
      $data["SOLDTONAME"] = $this->emptyColumnChecker($row["SOLDTONAME"], 1);
      $data["SALESPERSON"] = $this->emptyColumnChecker($row["SALESPERSON"], 1);
      $data["CUST_CONTNO"] = $this->emptyColumnChecker($row["CUST_CONTNO"], 1);
      $data["SHP_STREET_BRGY"] = $this->emptyColumnChecker($row["SHP_STREET_BRGY"], 1);
  ...
  public function get_cmf_data_via_id($db, $CMFID){

      $sql = "SELECT * from tblCMF where cID = '$CMFID'";
      $stmt= $db->query($sql);
      $data = $stmt->fetchALL(PDO::FETCH_OBJ);
      return $data;
  ...
  public function emptyColumnChecker($col, $stat){
    if($col == null || $col == 'null' || $col == '' || $col == ' '){
      if($stat == 1){
        return '---';
      }else{  
        return '';
  ...
  public function get_mor_salesodit($db, $start, $end){

    $sql = "SELECT a.mdCode,concat(mdSalesmanCode,' - ',mdName) Salesperson,a.custCode,
    b.custName,refno,sentDate,deliveryDate,totalAmount,a.longitude,a.latitude,versionNo,
    batteryStat,upTime 
    from tblTransaction a, tblcustomer b, tblUser c
    where convert(date,deliverydate) between '$start' and '$end' 
    and transstat = 0 and isVerified = 1 and a.custCode = b.custCode and a.mdcode = c.mdCode";
    $stmt= $db->query($sql);
    $data = $stmt->fetchALL(PDO::FETCH_OBJ);
    return $data;
  ...
  public function waveCell($phoneNumber, $message){
    $GlobHolder;
  ...
  public function insert_sweeper($db, $sweeperID, $mdCode){
    
    $statement = "INSERT INTO [tblSweeper] (sweeperID, mdCode, status, lastUpdated)
    values (?, ?, ?, getdate())";

    $sql = $db->prepare($statement);
    $result = $sql->execute([$sweeperID, $mdCode, 1]);

    return $result;

  }

  public function remove_sweeper($db, $sweeperID, $mdCode){
    
    $statement = "DELETE [tblSweeper] where sweeperID = ? and mdCode = ?";

    $sql = $db->prepare($statement);
    $result = $sql->execute([$sweeperID, $mdCode]);
  ...
  public function get_sweeper_accounts($db, $mdCode){

      $sql = "SELECT mdCode, sweeperID,
      (SELECT concat(mdSalesmancode, '_', mdName) as salesman from tbluser where tbluser.mdCode = tblSweeper.mdCode) as salesman
      from tblSweeper where sweeperID = '$mdCode'";
      $stmt= $db->query($sql);
      $data = $stmt->fetchALL(PDO::FETCH_OBJ);
  ...
  public function get_all_salesman_avl_sweeper($db){
    $sth = "SELECT distinct mdCode, concat(mdSalesmancode, '_', mdName) as salesman 
            from 
              tblUser";
      $result = array();
      $data = array();
        foreach ($db->query($sth) as $row) {
          $checker = $this->mdcode_sweeper_checker($db, $row["mdCode"]);
          if($checker == 0){
            $result["Salesman"] = $row["salesman"];
            $result["mdCode"] = $row["mdCode"];
            $data[] = $result;
  ...
  public function mdcode_sweeper_checker($db, $mdCode){
    $sql = $db->query("SELECT count(*) as checker from tblSweeper where mdCode = '$mdCode'");
    $retval = $sql->fetch();
    $res = $retval['checker'];
  ...
  public function exec_remove_sosyoStore_Tagging($db, $storeID){
    $statement = "UPDATE [TBLCUSTOMER_LS] set StoreName = null, custCode = 'NEW_CUSTOMER', linkName = null,
    partnerID = null, priceCode = null, status = 1 where storeID = ?";

    $sql = $db->prepare($statement);
    $result = $sql->execute([$storeID]);

    return $result;
  }

  public function get_pending_request($db, $start, $end, $appvID){

    $sql = "SELECT * from tblpendingrequest where cast(transDate as date) between '$start' and '$end' and ApproverContactNumber = '$appvID'";
    $f_Data = array();
    $data = array();
    foreach ($db->query($sql) as $row) {
        $data["cID"] = $row["cID"];
        $data["transType"] = $this->get_pr_stat($row["transType"]);
        $data["transDate"] = $row["transDate"];
        $data["mdCode"] = $row["mdCode"];
        $data["refNo"] = $row["refNo"];
        $data["Amount"] = $row["Amount"];
        $data["SKUCount"] = $row["SKUCount"];
        $data["VolumeQty"] = $row["VolumeQty"];
        $data["RequestorNote"] = $row["RequestorNote"];
        $data["ApproverContactNumber"] = $row["ApproverContactNumber"];
        $data["ApprovalCode"] = $row["ApprovalCode"];
        $data["ApprovalStatus"] = $this->get_pr_appv_Stat($row["ApprovalStatus"]);
        $data["ApproveDate"] = $this->check_pr_date($row["ApproveDate"]);
        $data["Remarks"] = $this->check_pr_date($row["Remarks"]);
        $data["lastUpdated"] = $row["lastUpdated"];
        $f_Data[] = $data;
    }
    return $f_Data;
  ...
  public function get_pr_stat($stat){
    if($stat == 'booking_approval'){
  ...
  public function get_pr_appv_Stat($stat){
    if($stat == null || $stat == ''){
      return 'PENDING';
    }
    return $stat;
  ...
  public function check_pr_date($stat){
    if($stat == null || $stat == ''){
      return '---';
    }
    return $stat;
  ...
  public function exec_update_pendingReq($db, $cID, $appvStat, $remarks){
    $statement = "UPDATE [tblpendingrequest] set ApprovalStatus = ?, Remarks = ?, ApproveDate = getdate(), lastUpdated = getdate() where cID = ?";

    $sql = $db->prepare($statement);
    $result = $sql->execute([$appvStat, $remarks, $cID]);

    return $result;
  }

  public function get_pendingreq_user($db, $phonenum){
    $statement = "SELECT count(*) as checker from tblPendingRequest where ApproverContactNumber = '$phonenum'";
  ...
  public function get_voucher_transaction($db_sosyo, $site_indicator, $start, $end){

    $convertedSite = '';

    if($site_indicator == 'EV00401'){
      $convertedSite = 'FDC_BOHOL';
    }else if($site_indicator == 'GM02201'){
      $convertedSite = 'FDC_QC';
    }else if($site_indicator == 'SM01901'){
      $convertedSite = 'FDC_GENSAN';
    }else if($site_indicator == 'WV00201'){
      $convertedSite = 'FDC_ROXAS';
    }else if($site_indicator == 'NM01401'){
      $convertedSite = 'FDC_CDO';
    }else if($site_indicator == 'EV00201'){
      $convertedSite = 'FDC_SAMAR';
    }else if($site_indicator == 'EV00301'){
      $convertedSite = 'FDC_LEYTE';
    }else if($site_indicator == 'Enc_OCSILaguna_N'){
      $convertedSite = 'OCSI LAGUNA';
    }

    $sql = "SELECT PROMOID, DATEUSED, a.TRANSACTIONID, c.pName, e.custCode, d.STORENAME, '50 pesos' as PROMOVAL,
              f.totalAmount
            from 
                TBLDISCOUNTTRANSACTION a, TBLSTRGEOSERVICE b, TBLPARTNER_DISTRIBUTOR c, tblstore d, tblCustomer_LS e,
                tbltransaction f
            where 
                a.CUSTOMERID = b.STOREID AND
                b.FENCE_ID = c.pID and
                a.CUSTOMERID = d.STOREID and
                a.CUSTOMERID = e.STOREID and
                a.TRANSACTIONID = f.transactionID and
                PROMOID = 'P50 sign-up voucher Promo' and 
                cast(a.dateused as date) between '$start' and '$end' and
                pName = '$convertedSite'
              order by
                  DATEUSED desc";
    
    $stmt= $db_sosyo->query($sql);
    $data= $stmt->fetchALL(PDO::FETCH_OBJ);
