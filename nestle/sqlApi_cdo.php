  <?php
include 'connectionString/cdoConnection.php';
include 'connectionString/cdoConnection_efast.php';
  ...
  if($_POST['type'] == "SyncCustomer_bohol"){
   ...
   // $mdCode = 'All';
    ...
    $sql = "SELECT upper(format(cast(deliverydate as date), 'ddd')) as nameofdate,
              ...
              [#SKU]
            from tblSellOutCur
            where cast(deliverydate as date) = '$date' and site = 'CDO'";
    ...
    foreach ($db->query($sql) as $row) {
      ...
      $current_date = date("Y-m-d");
       ...
       from vCategorySalesCur where cast(deliveryDate as date) between '$from' and '$to' and Site='CDO'
        group by Brand, BrandColor order by tAmount";
       $sth = $db_cloud->query("SELECT top 1 sum(totalAmount) as grandT from vCategorySalesCur where cast(deliveryDate as date) between '$from' and '$to' and Site='CDO'");
       $row = $sth->fetch(PDO::FETCH_ASSOC);
       $grandTotal = $row['grandT'];
       $percentage = 0;
       $data = array();
       ...
       Brand = '$productName' and Site='CDO'";
       $result = array();
       $data = array();
        foreach ($db_cloud->query($sql) as $row) {
           $result["deliveryDate"] = $row["deliveryDate"];
    ...
    $data = array();
      foreach($salesman as $resultRow){
        $sql = "Select Site,mdCode,Salesman,tcalls,pCalls,unpCalls,tsales,tSellHour,
              ...
              from (
                ...
                group by Site,mdCode
              ) x1 where mdCode = '$resultRow' and site = 'CDO'";
     ...
     $salesman = $_GET['resultSalesman'];
      ...
      foreach($salesman as $resultRow){
        $sql = "SELECT *
                FROM tblSellOutCur where mdCode = '$resultRow' AND CAST(deliveryDate as date) BETWEEN '$startDate' and '$endDate' and site = 'CDO'";
          /*$result = $db->prepare($sql);
          $result->execute(); 
          $number_of_rows = $result->fetchColumn();*/
   ...
   $salesman = $_POST['resultSalesman'];
    ...
    $sql = "SELECT * from [vStockCard] where cast(transDate as date) = '2018-08-07' and mdCode = '5020'";
      ...
      foreach ($db->query($sql) as $row) {
       ...
       $sql = "SELECT [Salesman]
                       ...
                       ,[TIME BOUND Min.]
                       ,[upTime]
                 FROM [dbo].[tblSellOutCur] WHERE CAST(deliveryDate as date) between '2018-08-01' and '2018-08-31' and site = 'CDO'";
       $f_Data = array();
       $data = array();
        foreach ($db_cloud->query($sql) as $row) {
           $data["mdCode"] = $row["mdCode"];
    ...
    echo json_encode($res);
      ...
      foreach ($db->query($sth) as $row) {
       ...
       $sth1 = $db_cloud->prepare("[dbo].[sp_datasync_cdo] @site = 'CDO'");
       $result1 = $sth1->execute();

       $sth2 = $db_cloud->prepare("[dbo].[sp_datasync1_cdo] @site = 'CDO'");
    ...
    // BETWEEN '$start' and '$end' order by trnDate desc";
     ...
     $sql = "SELECT * from vtblOverrides order by trnDate desc";
