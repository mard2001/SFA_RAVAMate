<?php
include 'connectionString/boholConnection.php';
include 'connectionString/boholConnection_efast.php';
 ...
 
{
  ...
  //             [#SKU]
  //           from tblSellOutCur
  //           where cast(deliverydate as date) = '$date' and site = 'BOH'";
  ...
  //      from vCategorySalesCur where cast(deliveryDate as date) between '$from' and '$to' and Site='BOH'
  //       group by Brand, BrandColor order by tAmount";
  //      $sth = $db_cloud->query("SELECT top 1 sum(totalAmount) as grandT from vCategorySalesCur where cast(deliveryDate as date) between '$from' and '$to' and Site='BOH'");
  //      $row = $sth->fetch(PDO::FETCH_ASSOC);
  //      $grandTotal = $row['grandT'];
  //      $percentage = 0;
  //      $data = array();
  ...
  //      Brand = '$productName' and Site='BOH'";
  //      $result = array();
  //      $data = array();
  //       foreach ($db_cloud->query($sql) as $row) {
  //          $result["deliveryDate"] = $row["deliveryDate"];
  ...
  //               group by Site,mdCode
  //             ) x1 where mdCode = '$resultRow' and site = 'BOH'";
  ...
  //     foreach($salesman as $resultRow){
  //       $sql = "SELECT *
  //               FROM tblSellOutCur where mdCode = '$resultRow' AND CAST(deliveryDate as date) BETWEEN '$startDate' and '$endDate' and site = 'BOH'";
  //         /*$result = $db->prepare($sql);
  //         $result->execute(); 
  //         $number_of_rows = $result->fetchColumn();*/
  ...
  //                      ,[TIME BOUND Min.]
  //                      ,[upTime]
  //                FROM [dbo].[tblSellOutCur] WHERE CAST(deliveryDate as date) between '2018-08-01' and '2018-08-31' and site = 'BOH'";
  //      $f_Data = array();
  //      $data = array();
  //       foreach ($db_cloud->query($sql) as $row) {
  //          $data["mdCode"] = $row["mdCode"];
  ...
  //      $sth1 = $db_cloud->prepare("[dbo].[sp_datasync_bohol] @site = 'BOH'");
  //      $result1 = $sth1->execute();

  //      $sth2 = $db_cloud->prepare("[dbo].[sp_datasync1_bohol] @site = 'BOH'");
  ...
  // }
    ...
    $sth = $db->prepare("[dbo].[sp_smsinsert] @mobileno= ?, @message= ?");
    $result = $sth->execute([$number, $message]);
