...
// getcompname();
getcompname_dynamic("RGS Report", "titleHeading");
function getcompname(){
    $.ajax ({
        ...
        cache: false,            
        success: function(r){ 
            $('#titleHeading').html(r[0].company.toUpperCase() +' | RGS Report');
        }
    });
}
...
function stockRequestSourceData(start, end){
    ...
    setTimeout(() => {
        $.ajax ({
            url: GLOBALLINKAPI+"/connectionString/applicationipAPI.php",
            type: "POST",
            data: {"type":"BOreports_RGS", "startDate":start, "endDate":end, "userID": GBL_USERID, "distCode": GBL_DISTCODE},
            dataType: "json",
            crossDomain: true,
            cache: false,  
            async: false,          
...
function datePicker(){
    ...
    $('#reportrange1').daterangepicker({
        ...
        if(start.format('MMMM D, YYYY') == end.format('MMMM D, YYYY')){
            ...
            $('#reportrange1 span').html(dayOfWeek + ' | '+start.format('MMMM D, YYYY'));
        } else{
...
function datatableApp(){
    tableData = $('#stockRequest_TAB').DataTable({
        ...
        columns: [
            { data: "salesmanCode", title: "Mdcode" },
            ...
            { 
                ...
                render: function(data, type, row){
                    if(row.quantity_ordered == null || row.quantity == null || row.ConvertionFactor == null){ return '---'}

                    // return row.quantity_ordered - row.quantity; 
                    return parseInt(row.quantity)/parseInt(row.ConvertionFactor); 
                } 
            },
            { 
                data: "PurchaseUom", 
