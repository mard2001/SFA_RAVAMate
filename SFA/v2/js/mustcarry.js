...
// getcompname();
getcompname_dynamic("Must Carry Maintenance", "titleHeading");
...
function getcompname(){
    $.ajax ({
        ...
        cache: false,            
        success: function(r){ 
            $('#titleHeading').html(r[0].company.toUpperCase() +' | Must Carry Maintenance');
        }
    });
} 
...
function datatableApp(){
    tableData = $('#stockRequest_TAB').DataTable({
        ...
        columns: [
            { data: "CUST_TYPE", title:"Customer Type" },
        ...
        buttons: [
            ...
            },
            {
                ...
                text: 'Excel',
                title: '',
                filename: 'MustCarry Maintenance - '+ moment().format("DD-MMM-YYYY"),
    ...
    function viewDetails(r){
        ...
        $('#customer_type').val(r.CUST_TYPE);
        $('#mustcarry_item').val(r.ITEM_NUMBER +' - '+ r.ITEM_DESCRIPTION);
...
function insert_cust_tieirng(){
    ...
    var f = confirm('Are you sure you want to add this must carry item?');
    if(f){
        $.ajax ({
        url: GLOBALLINKAPI+"/connectionString/applicationipAPI.php",
            type: "POST",
            data: {
                type: "INSERT_MUSTCARRY",
                custtype:custclass,
            ...
            success: function(r){ 
             if(r){
                alert('Successfully Inserted Must Carry Item');
             }
             stockRequestSourceData();
...
function delete_cust_tieirng(){
    ...
    if(f){
        ...
        url: GLOBALLINKAPI+"/connectionString/applicationipAPI.php",
            ...
            success: function(r){ 
             if(r){
                alert('Successfully Removed Must Carry Item');
             }
             stockRequestSourceData();
             $('#custtierdetailsModal').modal('hide');
            },
