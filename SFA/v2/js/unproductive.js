...
// getcompname();
getcompname_dynamic("Unproductive Report v.2.0.0", "titleHeading");
function getcompname(){
    $.ajax ({
        ...
        cache: false,            
        success: function(r){ 
            $('#titleHeading').html(r[0].company.toUpperCase() +' | Unproductive Report v.2.0.0');
        }
    });
} 
...
function stockRequestSourceData(start, end){
    ...
    $.ajax ({
        url: GLOBALLINKAPI+"/connectionString/applicationipAPI.php",
        type: "POST",
        data: {"type":"GET_UNPROD_REPORT", "startDate":start, "endDate":end, "userID": GBL_USERID, "distCode": GBL_DISTCODE},
        dataType: "json",
        crossDomain: true,
        cache: false,  
        async: false,          
...
function datatableApp(){
    tableData = $('#stockRequest_TAB').DataTable({
        ...
        columns: [
            { data: "trnDate", title: "Transaction Date" },
