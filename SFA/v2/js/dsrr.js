...
getcompname_dynamic("DSRR", "titleHeading");
// getcompname();
function getcompname(){
    $.ajax ({
        ...
        cache: false,            
        success: function(r){ 
            $('#titleHeading').html(r[0].company.toUpperCase() +' | DSRR');
            ...
            didOpen: () => {
                ...
                setTimeout(() => {
                    ...
                    $.ajax ({
                        url: GLOBALLINKAPI+"/connectionString/applicationipAPI.php",
                        type: "POST",
                        data: {"type":"dsrr_data", "dsrSalesman": salesman, "dsrDate":date, "userID": GBL_USERID, "distCode": GBL_DISTCODE},
                        dataType: "json",
                        crossDomain: true,
                        cache: false,            
                        success: function(response){ 
                            generatePrintableReport(response.tabular_json_content);
                            $('#printThisTable').hide();
...
function generatePrintableReport(jsonData) {
    var bodyContent = '';  
    var minRows = 30;  
    var totalRows = Math.max(jsonData.length, minRows);
    totalGross = 0;
    ...
    for (var x = 0; x < totalRows; x++) {
        ...
        if (x < jsonData.length) {
            row = `
                <tr>
                    <td>${jsonData[x].count}</td>
                    <td>${jsonData[x].refno}</td>
                    <td class="text-start">${jsonData[x].Customer}</td>
                    <td class="text-start">${jsonData[x].address}</td>
                    <td class="text-end">${(jsonData[x].GrossAmt).toLocaleString('en-US', { minimumFractionDigits: 4, maximumFractionDigits: 4 })}</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td class="text-end">${jsonData[x].Sales}</td>
                </tr>
            `;

            if (!isNaN(jsonData[x].GrossAmt)) {
                // Round UP to 4 decimal places
                var grossAmt = Math.ceil(jsonData[x].GrossAmt * 10000) / 10000;
                totalGross += grossAmt;
            }
        } else {
            row = `
    ...
    $('#dsrSalesmanData').html(bodyContent);
    $('#gross_amt').html(totalGross.toLocaleString('en-US', { minimumFractionDigits: 4, maximumFractionDigits: 4 }));
}
