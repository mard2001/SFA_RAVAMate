...
getcompname_dynamic("Fastsosyo Calendar | v.1.8", "titleHeading");
// getcompname();
function getcompname(){
    $.ajax ({
        ...
        success: function(r){ 
            ...
            $('#ed_cutofftime').val(r[0].SOSYOCUTOFFTIME);
            $('#taggedsosyocutoff').html(r[0].SOSYOCUTOFFTIME);
            $('#titleHeading').html(r[0].company.toUpperCase() +' | Fastsosyo Calendar | v.1.8');
        },error: function(jqXHR, textStatus, errorThrown) {
            alert('ERROR CONNECTING TO SERVER:\n Please Check your connection settings.');
        }
    });
...
function exec_add_calendar(){
    // var add_eventtype = $('#add_eventtype').val();
    // var add_date = $('#dssdate').val();
    
    var eventDescription = moment(dssdate);
    var formattd = eventDescription.format('dddd');

    var f = confirm('Are you sure you want to add '+dssdate+' as non-selling days?');
      if(f){
        $.ajax ({
          url: GLOBALLINKAPI+"/connectionString/applicationipApi.php",
          type: "POST",
          data: {
                  "type":"ADD_SOSYO_CALENDAR", 
                  'distributorCode':$.trim(localStorage.getItem('DISTCD')),
                  'eventDate':dssdate,
                  'eventDescription':formattd,
                  "userID": GBL_USERID,
                  "distCode": GBL_DISTCODE
                },
          dataType: "JSON",
          ...
          async: false,       
            success: function(r){ 
              if(r){
                alert('Date was successfully added.');
              }
...
function customerDataTable(){
    tableData =  $('#stockRequest_TAB').DataTable({
        ...
        "columns": [
            ...
            { "data": "DistributorCode", title: "Distributor Code"},
            { "data": "EventDate", title: "Event Date"},
            { "data": "EventDescription", title: "Day" },
    ...
    function exec_savetojobber(r){
        ...
        $('#d_eventtype').html(r.EventType);
        $('#d_distcode').html(r.DistributorCode);
        $('#d_date').html(r.EventDate);
        $('#d_day').html(r.EventDescription);
        $('#calendarID').val(r.cID);
...
function remove_calendar_day(){
    var cID = $('#calendarID').val();
    var f = confirm('Are you sure you want to remove this date?');
      if(f){
        $.ajax ({
          url: GLOBALLINKAPI+"/connectionString/applicationipApi.php",
          type: "POST",
          data: {
                  "type":"REMOVE_SOSYO_CALENDAR", 
                  "cID":cID,
                  "userID": GBL_USERID,
                  "distCode": GBL_DISTCODE
                },
          ...
          async: false,       
            success: function(r){ 
              if(r){
                alert('Date was successfully removed.');
              }
...
function sosyocutofftime(){
    $('#sosyocutoffmodal').modal('show'); 
}
