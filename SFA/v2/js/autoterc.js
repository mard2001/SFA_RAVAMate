...
// getcompname();
getcompname_dynamic("Auto Stock Transfer to ERIC logs", "titleHeading");
...
function getcompname(){
    $.ajax ({
        ...
        cache: false,            
        success: function(r){ 
            $('#titleHeading').html(r[0].company.toUpperCase() +' | Auto Stock Transfer to ERIC logs');
        }
    });
} 
...
function stockRequestSourceData(start, end){
    ...
    $.ajax ({
        ...
        type: "GET",
        data: {
            type: "GET_ERIC_AUTOTRANSFER_SENDING_LOGS",
            CONN: con_info,
            startdate: start,
            enddate: end,
        },  
