...
getcompname_dynamic("Fastsosyo Activations | v.1.8", "titleHeading");
// getcompname();
function getcompname(){
    $.ajax ({
        ...
        success: function(r){ 
            ...
            $('#titleHeading').html(r[0].company.toUpperCase() +' | Fastsosyo Activations | v.1.8');
        },error: function(jqXHR, textStatus, errorThrown) {
            alert('ERROR CONNECTING TO SERVER:\n Please Check your connection settings.');
        }
    });
...
function customerDataTable(){
    tableData =  $('#stockRequest_TAB').DataTable({
        ...
        "columns": [
            ...
            { "data": "MD_SALESMAN", title: "Salesman" },
            { "data": "ADDRESS", title: "Adrress"},
    ...
    function exec_savetojobber(r){
        console.log(r);
        if(r.DOWNLOADED_MYBUDDY_STAT == null){
            $('#dlorder').show();
            $('.scorder').hide();
        }else{
            $('#dlorder').hide();
