...
var fDirectionsService = new google.maps.DirectionsService;
var fDirectionsDisplay = new google.maps.DirectionsRenderer;
...
var GBLAPURL = "https://fastdevs-api.com/MYBUDDYGLOBALAPI";
...
showsitescon();
function showsitescon() {
  var fdcchecker = GBLCOMPNAMEHOLDER;
  // console.log(fdcchecker.substring(0, 3));

  $('#avsiteSelect').html('<option seleted disabled hidden>Application not available.</option>');
  if (fdcchecker == 'FDC') {
    $.ajax({
      url: GBLAPURL + "/nestle/connectionString/applicationipAPI.php",
      type: "GET",
      data: { "type": 'GET_AVAILABLE_SITESCON', "CONN": con_info },
      dataType: "json",
      crossDomain: true,
      cache: false,
      success: function (r) {
        ...
        if (r.length == 0) {
          ...
          cont = `<option seleted disabled hidden value=''>Choose here..</option>'`;
          for (var x = 0; x < r.length; x++) {
            cont += '<option value=' + r[x].cID + '>' + r[x].SITENAME + '</option>';
          }
          $('#avsiteSelect').html(cont);
        }
      }
    });
  } else {
    $.ajax({
      url: GLOBALLINKAPI + "/nestle/connectionString/applicationipAPI.php",
      type: "GET",
      data: { "type": 'GET_AVAILABLE_SITESCON_FDC', "CONN": con_info },
      dataType: "json",
      crossDomain: true,
      cache: false,
      success: function (r) {
...
function setupChecker() {
  ...
  $.ajax({
    url: GLOBALLINKAPI + "/nestle/connectionString/applicationipAPI.php",
    type: "GET",
    data: { "type": "CHECK_SITE_SETUP", "CONN": con_info },
    dataType: "json",
    crossDomain: true,
    cache: false,
    success: function (r) {
...
// var dupUserChecker = new IntervalTimer(function () {
//   check_user_usage_stat();
//   }, 2000);  
      ...
      if (data == 0) {
        ...
        for (var x = 0; x < data.length; x++) {
          ...
          var contentString = "<div style='margin: 0 auto;'><div class='modal-header' style='text-align: center;'>" +
            "<div class=''><img class='salesmanPic' alt='salesmanPic' style='border: solid 3px " + data[x].mdColor + "' src='" + imglink + data[x].mdCode + ".jpg' onError='imgError(this)'/>" +
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (count != delimeter) {
        ...
        // $('#floatTab tr:first-child th:first-child').text("All Salesman");
      }
    ...
    success: function (data) {
      ...
      if (data.length == 0) {
        ...
        for (var x = 0; x < data.length; x++) {
          ...
          checkWithinMPA(data[x].latitude, data[x].longitude, data[x].Salesman, data[x].Sales, data[x].deliveryDate, data[x].Customer, data[x].DocumentNo);
          // var imageThumbnail = getImage_local(data[x].mdCode);
          compDist = distance(data[x].latitude, data[x].longitude, data[x].transCount);
          // src='data:image/jpeg;base64,"+data[x].thumbnail+"' --not base64 img/erw.jpg
          var contentString = "<div style='margin: 0 auto;'><div class='modal-header' style='text-align: center;'>" +
            "<div class=''><img class='salesmanPic' alt='storeimage' style='border: solid 3px " + data[x].mdColor + "' src='" + imglink + data[x].customerID + ".jpg' onError='imgError(this)'/>" +
            "<br/><span class='label label-pill label-danger count' style='font-size: 10px; border-radius:100%; float:center; background-color:" + data[x].mdColor + ";' id='getThisTransCount'>" + data[x].transCount + "</span>" +
            "<span class='fa fa-chevron-left pull-left' style='float: left; font-size: 30px;' onclick='prevInfo()' id='prevMD'></span><span id='nextMD' onclick='nextInfo()' class='fa fa fa-chevron-right pull-right' style='float: left; font-size: 30px;'></span></div></div>" +
            "<div class='table-responsive'>" +
            "<table class='table table-condensed'>" +
      ...
      if (response == 0) {
        ...
        for (var i = 0; i < mpa_header.length; i++) {
          ...
          for (var j = 0; j < mpa_header[i][8].length; j++) {
            ...
            coord_ary.push({
              lat: Number(mpa_header[i][8][j].MPALATITUDE),
...
function saveEncroachment(boundOwner, salesman, sales, date, customer, lat, long, boundary, refno) {
  ...
  $.ajax({
    ...
    type: "GET",
    data: {
      "type": "encruchment_Save_" + user, "boundOwnerEncruch": boundOwner, "salesmanEncruch": salesman, "salesEncruch": sales, "dateEncruch": date,
...
function init() {
  ...
  function onSuccess() {
    ...
    displayAllData = new IntervalTimer(function () {
      if ($('.container_salesman').is(':visible')) {
        storeTblValues();
      }
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        if (i < 22) {
          ...
          if (response[i].alert == 'EARLY') {
            ...
            txt += `<td class='expand'><img onclick='remarkSalesmanClick("${response[i].salesmanName}", "${response[i].refNo}", "${response[i].deliveryDate}", "${response[i].TransTime}", "${response[i].alert}", "${response[i].mobileNo}", "${response[i].mdCode}", "${response[i].mdColor}", "${response[i].customerLoc}", "${response[i].latLng}", "${response[i].thumbnail}", "${response[i].calltime}")' data-toggle='tooltip' title="${response[i].salesmanName}" id='late' alt='${response[i].salesmanName}' src="${imglink}${response[i].mdCode}.jpg" onError='imgError(this)'/></td>`;
...
function getSalesmanImageServer() {
  $.ajax({
    url: GLOBALLINKAPI + "/nestle/connectionString/applicationipAPI.php",
    type: "GET",
    data: { "type": "GET_SALESMAN_IMAGES", "CONN": con_info },
    dataType: "json",
    crossDomain: true,
    async: true,
    cache: false,
...
function getImage_local(mdCode) {

  // var r = GLOBALIMGHOLDER;
  // // console.log(mdCode);
  // // console.log(GLOBALIMGHOLDER.length);
  // var res = '';
  // for(var x = 0; x < r.length; x++){
  //   if(mdCode == r[x].mdCode){
  //     res = r[x].thumbnail;
  //     // console.log('mdcode img found. ' + r[x].mdCode);
  //   }
  // }
...
function remaining() {
  ...
  for (var x = 0; x < data.length; x++) {
    ...
    cont += "<tr onclick='showRemaining(\"" + data[x].mdCode + "\")'><td>" + (x + 1) + " <img class='remainingDevPics' src='" + imglink + data[x].mdCode + ".jpg' onError='imgError(this)'/> " + data[x].mdCode + '_' + data[x].salesmanName + " " + alert + "</td></tr>";
  }
...
function tranferSite() {
  var siteLoc = $('#avsiteSelect').val();
  if (siteLoc != undefined || siteLoc != 'undefined') {
    $('#transBtn').html("<i class='fa fa-spin fa-spinner'></i> please wait..");
    $.ajax({
      url: GBLAPURL + "/nestle/connectionString/applicationipAPI.php",
      type: "GET",
      data: { "type": "GET_SELECTED_AVAILABLE_SITESCON", "CONN": con_info, "siteLoc": siteLoc },
      dataType: "json",
      crossDomain: true,
      cache: false,
      success: function (r) {
        if (r.length == 0) {
          ...
          var encryptedS = CryptoJS.AES.encrypt(r[0].HOSTNAME, "/");
          var encryptedU = CryptoJS.AES.encrypt(r[0].USERID, "/");
          var encryptedP = CryptoJS.AES.encrypt(r[0].PASS, "/");
          var encryptedD = CryptoJS.AES.encrypt(r[0].DATABASE, "/");
        ...
        $('#transBtn').html("Transfer");
      }, error: function (jqXHR, textStatus, errorThrown) {
        alert('ERROR CONNECTING TO SERVER:\n Please Check your connection settings.');
      }
    });
  } else {
    alert('Please select a site to transfer.');
  }
...
function setDatatables() {
  ...
  dataTables = new DataTable('#floatTab', {
    lengthChange: true,
    pageLength: 22,
    order: false,
    // bSort: false,
...
function setStyles() {
  //override lengthChange element
  var a = `<div id="dropDownSalesmanCategory" class="dropdown" style="color: black; margin-left: 5px;">
    <button class="btn btn-naiCol btn-xs dropdown-toggle salesmanCatIndi" type="button"
      data-toggle="dropdown"> ${$('#floatTab tr:first-child th:first-child').text()}
  ...
  $('#floatTab tr:first-child th:nth-child(2)').text("Calls");
  $('#floatTab tr:first-child th:nth-child(3)').text("Unproductive");
  $('#floatTab tr:first-child th:nth-child(4)').text("Selling Hours");
  $('#floatTab tr:first-child th:nth-child(5)').text("Sales");

  $('#floatTab_wrapper .dt-layout-row:first-child').css({
    'padding-right': '7px'
  });
...
function showPosition(position) {
  ...
  var targetSalesman = $('#salesman_name').html();

  var findLastLocation = globalPost.filter(marker => {
    return marker.Salesman == targetSalesman;

  });


  var targetLocation = new google.maps.LatLng(findLastLocation[findLastLocation.length - 1].latitude, findLastLocation[findLastLocation.length - 1].longitude);


  fDirectionsDisplay.setMap(map);
  fDirectionsDisplay.setOptions({ suppressMarkers: true });
  calculateAndDisplayRoute(fDirectionsService, fDirectionsDisplay);

  function calculateAndDisplayRoute(fDirectionsService, fDirectionsDisplay) {
    fDirectionsService.route({
      origin: currentLocation,
      destination: targetLocation,
      travelMode: 'DRIVING'
    }, function (response, status) {
      if (status === 'OK') {
        fDirectionsDisplay.setDirections(response);
